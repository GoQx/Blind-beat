...
const ipfs = ipfsAPI({ ip: 'localhost', port: '5001', protocol: 'http' })
...
const App = {
  start: async function () {
    ...
    $('#bidding').submit(function (event) {
      ...
      console.log(`理想价格: ${bidAmount}, 迷惑价格: ${bidSend}, 密文: ${secretText}`)

      // 这个函数等同于solidity中的keccak256函数
      let byteHash = web3.utils.soliditySha3(bidAmount, secretText)

      console.log('byteHash:', byteHash)

      web3.eth.getAccounts().then(accounts => {
        console.log('accounts :', accounts)
        // ecommerceStoreInstance.bid(parseInt(productId), bidAmount, secretText, {
        // 生成哈希

        ecommerceStoreInstance.bid(parseInt(productId), byteHash, {
          from: accounts[0],
          value: bidSend
        }).then(res => {
          console.log(res)
    ...
    $('#revealing').submit(function (event) {
      ...
      // ecommerceStoreInstance.testHash.call(bigNumberAmount.toNumber(), secretText).then(res => {
    ...
    $('#add-item-to-store').submit(function (event) {
      ...
      saveProduct(reader, decodeParas) // 既包含了文本信息，又包含了图片信息，准备上传到ipfs和以太坊
      event.preventDefault()
    })
...
async function saveProduct (reader, parameters) {
  ...
  let imageHash = await saveImageToIpfs(reader) // 把图片字节流上传到ipfs, hash1
  console.log('ipfs imageHash : ', imageHash)
  let descHash = await saveDescInfoToIpfs(parameters['product-description']) // 把文本描述信息上传到ipfs, hash2
  console.log('ipfs descHash : ', descHash)
  // 将哈希等添加到区块链
  saveProductToBlockChain(parameters, imageHash, descHash) // 调用addProductToStore方法
}
...
async function renderProductDetail (productid) {
  // 3. 填充基本信息：图片，名字，起拍价格，起拍/截止时间等待
  let productInfo = await ecommerceStoreInstance.getProductById(productid)
  ...
  // 显示文本（描述信息）// ipfs
  let descInfo = await ipfs.cat(descLink)
  console.log('descInfo:', descInfo)

  // 把描述信息展示到对应的节点上
  $('#product-desc').append(`<div>${descInfo}</div>`)
  // 展示其他的辅助信息
  // - 辅助信息
  ...
  // 显示商品拍卖详情, 整个product.html的控件的显示控制也在这里

  // $('#bidding, #revealing, #finalize-auction, #escrow-info').hide()
  let currentTime = getCurrentTimeInSeconds()

  // 产品竞标情况status ： 0--> 进行中Open  1---> 卖掉Sold  2---> 没卖掉Unsold
  // 从这里入手！！
  if (parseInt(status) === 0) {
    ...
    ecommerceStoreInstance.getHighestBidInfo.call(id).then(info => {
      ...
      $('#product-status').html(`<p>产品状态：揭标已结束，最高价：${highestBid}, 开始进入仲裁投票阶段!</p>`)
    })
...
async function renderProducts () {
  // 1. 获取商品数量 10
  let productIndex = await ecommerceStoreInstance.productIndex()
  ...
  for (let i = 1; i <= productIndex; i++) {
    ...
    // 3. 填充基本信息：图片，名字，起拍价格，起拍/截止时间等待
    let productInfo = await ecommerceStoreInstance.getProductById(i)
    ...
    node.append(`<img src="http://localhost:8848/ipfs/${imageLink}" width="400px"/>`)
    ...
    node.append(`<a href="product.html?id=${id.toNumber()}">Details</a>`)
...
window.addEventListener('load', function () {
  if (typeof web3 !== 'undefined') {
    console.log('found inject web')
    // Use Mist/MetaMask's provider
    window.web3 = new Web3(window.web3.currentProvider)
    console.log('version111:', window.web3.version)
  } else {
    console.log('use 127.0.0.1')
